...
{    
    for (int i = 0; i < 10; i++)
    {
        ...
        obj->m_Position = { (float)(rand() % 800), (float)(rand() % 600) }; // obj 위치설정
...
void SetDynamicObject(TQuadtree& quadtree)
{
    for (int i = 0; i < 10; i++)
    {
        ...
        TPoint d = {  (float)randstep(0, 800), //이동할 포지션 설정하고
                     (float)randstep(0, 600) };
        obj->SetTarget(d); //방향 설정
...
{    
    ...
    {
        timer.Frame();       
        quadtree.PreFrame(); //전에 위치 오브젝트들 삭제

        for (int i = 0; i < DynamicObjectList.size(); i++)
        {
            TObject* obj = DynamicObjectList[i];
            obj->Move(timer.m_fSecondPerFrame); //이동
            quadtree.DynamicAddObject(obj); //이동할만큼 obj추가
        }
        
        quadtree.Frame(); //아직 구현 x
        quadtree.Render();  //아직 구현 x

        system("cls");
        std::cout << std::endl;
        std::cout << "Object inform!\n";
        quadtree.LevelOrder(quadtree.GetRootNode()); //순회 출력
        timer.Render();
