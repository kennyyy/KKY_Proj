...
public:
	...
	static bool RectToPoint(Rect& rt, Vector2& p){
		if (rt.mMin.x <= p.x && rt.mMax.x >= p.x && rt.mMin.y <= p.y && rt.mMax.y >= p.y)
			return true;
	...
	static bool BoxToPoint(Box& bx, Vector3& p) {
		if (bx.mMin.x <= p.x && bx.mMax.x >= p.x && bx.mMin.y <= p.y && bx.mMax.y >= p.y && bx.mMin.z <= p.z && bx.mMax.z >= p.z)
			return true;
